...
class StreamFlowPlayer {
  constructor() {
    ...
    this.youtubeFrame = document.getElementById("youtubePlayer");
  ...
  extractProxiedTargetUrl(possibleProxyUrl) {
    try {
      const parsed = new URL(possibleProxyUrl, window.location.origin);
      if (!parsed.pathname || !parsed.pathname.endsWith("/proxy")) return null;

      const target = parsed.searchParams.get("url");
      if (!target) return null;

      // URLSearchParams decodes once; older links may be double-encoded.
  ...
  buildYouTubeEmbedUrl(url) {
    ...
    // Some list types (notably RD* "mix" lists) commonly fail in embeds.
  ...
  init() {
    ...
    if (videoUrl) {
      // URLSearchParams already decodes, but older links may be double-encoded.
      // Decode defensively (without throwing) and only as needed.
      ...
      if (/%3A%2F%2F/i.test(normalizedUrl)) {
        try {
          normalizedUrl = decodeURIComponent(normalizedUrl);
        } catch {
          // ignore
      ...
      if (this.useProxyCheckbox && proxyParam === "1") {
  ...
  loadVideo() {
    const rawInputUrl = this.urlInput.value.trim();
    if (!rawInputUrl) {
      this.urlInput.focus();
      return;
    }

    // If the user pasted a local proxy URL, unwrap the real target URL.
    const proxiedTargetUrl = this.extractProxiedTargetUrl(rawInputUrl);
    const detectionUrl = proxiedTargetUrl || rawInputUrl;

    // YouTube links are not direct video streams, so play via embed instead of <video>.
    if (this.isYouTubeUrl(detectionUrl)) {
      ...
      // Proxy doesn't apply to YouTube embeds.
      if (this.useProxyCheckbox) this.useProxyCheckbox.checked = false;
      this.originalUrl = detectionUrl;
      this.currentUrl = embedUrl;
      // Normalize the input box so the user sees the real URL (not /proxy?...)
      this.urlInput.value = detectionUrl;
      this.showPlayerSection();
      this.showYouTube(embedUrl);

      const newUrl = new URL(window.location.href);
      newUrl.searchParams.set("url", detectionUrl);
      newUrl.searchParams.delete("proxy");
    ...
    this.originalUrl = detectionUrl; // Store original for display and for reloads

    // If the user pasted a full proxy URL, use it directly as the playback URL.
    let playbackUrl = proxiedTargetUrl ? rawInputUrl : detectionUrl;

    // Check if proxy should be used
    const useProxy = this.useProxyCheckbox && this.useProxyCheckbox.checked;
    if (useProxy) {
      // Use same-origin proxy server (run server.js with node)
      // Optional override: window.STREAMFLOW_PROXY_ORIGIN = 'http://localhost:4000'
      const proxyBase =
    ...
    // Check if HLS stream
    if (detectionUrl.includes(".m3u8")) {
      this.loadHLS(playbackUrl);
    } else if (detectionUrl.includes(".mpd")) {
      this.loadDASH(playbackUrl);
    } else {
      // Direct video URL - try loading with best settings for streaming
      this.loadDirectVideo(playbackUrl);
    }

    // Update URL params
    const newUrl = new URL(window.location.href);
    newUrl.searchParams.set("url", detectionUrl);
    if (useProxy && !proxiedTargetUrl) {
      newUrl.searchParams.set("proxy", "1");
    } else {
      newUrl.searchParams.delete("proxy");
    }
    window.history.replaceState({}, "", newUrl);
